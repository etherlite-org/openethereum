...
impl CliqueTester {
    ...
    pub fn with(epoch: u64, period: u64, initial_signers: Vec<char>) -> Self {
        ...
        clique
            .genesis_epoch_data(&genesis, &call)
            .expect("Create genesis failed");
        Self {
            clique,
            genesis,
            db: get_temp_state_db(),
            signers,
        }
    }
    ...
    pub fn new_block_and_import(
...

        {
            let difficulty = self.get_difficulty(
                block.header.number(),
                last_header,
                &self.signers[&signer].address(),
            );
            let b = block.block_mut();
            b.header
                .set_timestamp(last_header.timestamp() + self.clique.period);
...
fn one_signer_with_no_votes() {
    ...
    let empty_block = tester
        .new_block_and_import(CliqueBlockType::Empty, &tester.genesis, None, 'A')
        .unwrap();
...
fn one_signer_two_votes() {
    ...
    let vote = tester
        .new_block_and_import(
            CliqueBlockType::Vote(VoteType::Add),
            &tester.genesis,
            Some(tester.signers[&'B'].address()),
            'A',
        )
        .unwrap();
    ...
    let empty = tester
        .new_block_and_import(CliqueBlockType::Empty, &vote, None, 'B')
        .unwrap();
...
fn two_signers_six_votes_deny_last() {
    ...
    let empty = tester
        .new_block_and_import(CliqueBlockType::Empty, &prev_header, None, 'C')
        .unwrap();
...
fn one_signer_dropping_itself() {
    ...
    let vote = tester
        .new_block_and_import(
            CliqueBlockType::Vote(VoteType::Remove),
            &tester.genesis,
            Some(tester.signers[&'A'].address()),
            'A',
        )
        .unwrap();
...
fn vote_add_concurrently_is_permitted() {
    ...
    // Empty block signed by `B`
    let b = tester
        .new_block_and_import(CliqueBlockType::Empty, &b, None, 'B')
        .unwrap();
    ...
    let b = tester
        .new_block_and_import(CliqueBlockType::Empty, &b, None, 'A')
        .unwrap();
...
fn vote_rm_concurrently_is_permitted() {
    ...
    // Empty block signed by `B`
    let block = tester
        .new_block_and_import(CliqueBlockType::Empty, &block, None, 'B')
        .unwrap();
    // Empty block signed by `C`
    let block = tester
        .new_block_and_import(CliqueBlockType::Empty, &block, None, 'C')
        .unwrap();
    ...
    // Empty block signed by `A`
    let block = tester
        .new_block_and_import(CliqueBlockType::Empty, &block, None, 'A')
        .unwrap();
...
fn consensus_out_of_bounds_consensus_execute_on_touch() {
    ...
    assert_eq!(
        &tags,
        &['A', 'B', 'C'],
        "D should have been removed after 3/4 remove votes"
    );
...
fn pending_votes_doesnt_survive_authorization_changes() {
    ...
    assert_eq!(
        &tags,
        &['A', 'B', 'C', 'D', 'E', 'F'],
        "F should have been added"
    );

    // Vote against `F` from [`D`, `E`, `B`, `C`]
    for sign in SIGNER_TAGS
        .iter()
        .skip(3)
        .chain(SIGNER_TAGS.iter().skip(1).take(2))
    {
        let block = tester
            .new_block_and_import(
                CliqueBlockType::Vote(VoteType::Remove),
    ...
    assert_eq!(
        &tags,
        &['B', 'C', 'D', 'E', 'F'],
        "F should have been added again"
    );
}
...
fn epoch_transition_reset_all_votes() {
    ...
    let block = tester
        ...
        .unwrap();
    let block = tester
        .new_block_and_import(CliqueBlockType::Checkpoint, &block, None, 'A')
        .unwrap();
    ...
    assert_eq!(
        &tags,
        &['A', 'B'],
        "Votes should have been reset after checkpoint"
    );
}
...
fn unauthorized_signer_should_not_be_able_to_sign_block() {
    ...
    let err = tester
        .new_block_and_import(CliqueBlockType::Empty, &tester.genesis, None, 'B')
        .unwrap_err();
...
fn bonus_consensus_should_keep_track_of_votes_before_latest_per_signer() {
    ...
    // Empty block signed by `C`
    let vote = tester
        .new_block_and_import(CliqueBlockType::Empty, &vote, None, 'C')
        .unwrap();

    // Empty block signed by `D`
    let vote = tester
        .new_block_and_import(CliqueBlockType::Empty, &vote, None, 'D')
        .unwrap();
    ...
    // Empty block signed by `A`
    let vote = tester
        .new_block_and_import(CliqueBlockType::Empty, &vote, None, 'A')
        .unwrap();
